    ...
    public class CategoryReprository : ICategoryReprositry
    {
        ...
        public void Update(EditCategoryViewModel editCategoryViewModel)
        {
            ...
            if (uniqueFileName is null)
            {
                // chang name 
                string uploadsFolder = Path.Combine(webHostEnvironment.WebRootPath, "CategoryImages");
                string oldPath = Path.Combine(uploadsFolder, category.Image);
            ...
            else if (category.Name != editCategoryViewModel.Name && category.Image != uniqueFileName && uniqueFileName is not null)
            {
                string uploadsFolder = Path.Combine(webHostEnvironment.WebRootPath, "CategoryImages");
                var filePath = Path.Combine(uploadsFolder, category.Image);
                if (System.IO.File.Exists(filePath))
                {
                    System.IO.File.Delete(filePath);
                }
